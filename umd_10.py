...
def checkFileExistance(filePath):
    try:
        ...
        return False
if checkFileExistance('/content/gdrive/My Drive/BancoDados/UMD_10_num.dat'):
    import sys
...
# UMD_10 : lista que contará os dados para estatística dos classificadores
...
UMD_10_num = np.zeros(20, dtype=np.float64)
UMD_10_num[0] = best_acc
UMD_10_num[1] = xensemb
xvencedor=[]
tg = 2
# a seguir salvaremos os melhores scores obtidos
for z in range(3):
  UMD_10_num[tg] = xval[z]
  
  tg += 1
for z in range(3):
  UMD_10_num[tg] = xscor1[z]
  
  tg += 1
for z in range(3):
  UMD_10_num[tg] = et_scor[z]
  
  tg += 1
for z in range(3):
  UMD_10_num[tg] = es_scor[z]
  
  tg += 1
for z in range(3):
  UMD_10_num[tg] = mt_scor[z]
  
  tg += 1
for z in range(3):
  UMD_10_num[tg] = ms_scor[z]
  tg += 1
...
bs_total = as_total - 2
UMD_10_class = np.empty(bs_total, dtype=np.float64)
for i in range(bs_total):
    if xvencedor[i] == xnome_class[0]:
        UMD_10_class[i] = float(0.0)
    if xvencedor[i] == xnome_class[1]:
        UMD_10_class[i] = float(1.0)
    if xvencedor[i] == xnome_class[2]:
        UMD_10_class[i] = float(2.0)
    if xvencedor[i] == xnome_class[3]:
        UMD_10_class[i] = float(3.0)
    if xvencedor[i] == xnome_class[4]:
        UMD_10_class[i] = float(4.0)
    if xvencedor[i] == xnome_class[5]:
        UMD_10_class[i] = float(5.0)
    if xvencedor[i] == xnome_class[6]:
        UMD_10_class[i] = float(6.0)
    if xvencedor[i] == xnome_class[7]:
        UMD_10_class[i] = float(7.0)
    if xvencedor[i] == xnome_class[8]:
        UMD_10_class[i] = float(8.0)
    if xvencedor[i] == xnome_class[9]:
        UMD_10_class[i] = float(9.0)
cs_total = as_total - 2
UMD_10_qtd = np.zeros(18, dtype=np.float64)
UMD_10_qtd[0]=n_prim
UMD_10_qtd[1]=n_seg
UMD_10_qtd[2]=n_ter
UMD_10_qtd[3]=xn_prim
UMD_10_qtd[4]=xn_seg
UMD_10_qtd[5]=xn_ter
UMD_10_qtd[6]=et_n_prim
UMD_10_qtd[7]=et_n_seg
UMD_10_qtd[8]=et_n_ter
UMD_10_qtd[9]=es_n_prim
UMD_10_qtd[10]=es_n_seg
UMD_10_qtd[11]=es_n_ter
UMD_10_qtd[12]=mt_n_prim
UMD_10_qtd[13]=mt_n_seg
UMD_10_qtd[14]=mt_n_ter
UMD_10_qtd[15]=ms_n_prim
UMD_10_qtd[16]=ms_n_seg
UMD_10_qtd[17]=ms_n_ter
pprint(UMD_10_num)
pprint(UMD_10_class)
pprint(UMD_10_qtd)

arq_est_num = "/content/gdrive/My Drive/BancoDados/UMD_10_num.dat"
arq_est_class = "/content/gdrive/My Drive/BancoDados/UMD_10_class.dat"
arq_est_qtd = "/content/gdrive/My Drive/BancoDados/UMD_10_qtd.dat"
UMD_10_num.tofile(arq_est_num)
UMD_10_class.tofile(arq_est_class)
UMD_10_qtd.tofile(arq_est_qtd)

a_num = np.fromfile(arq_est_num)
b_class = np.fromfile(arq_est_class)
c_qtd = np.fromfile(arq_est_qtd)
pprint(a_num == UMD_10_num)
pprint(b_class == UMD_10_class)
pprint(c_qtd == UMD_10_qtd)
